...
export function shake(sprite: Phaser.Sprite) {
  const { game, x } = sprite;
  const start = { x };
...
export function excite(sprite: Phaser.Sprite) {
  return sprite.game.tweens
    .create(sprite)
    .to(
      { rotation: EXCITEMENT_SWING_ROTATION },
      EXCITEMENT_SWING_DURATION,
...
export function fadeIn(sprite: Phaser.Sprite) {
  sprite.game.tweens.create(sprite).to({ alpha: 1 }, FADE_DURATION).start();
}

export function fadeOut(sprite: Phaser.Sprite) {
  sprite.game.tweens.create(sprite).to({ alpha: 0 }, FADE_DURATION).start();
}

export function slide(sprite: Phaser.Sprite, position: {x: number, y: number}) {
  sprite.game.tweens
    .create(sprite)
...
export function fallIn(sprite: Phaser.Sprite) {
  const { game } = sprite;
